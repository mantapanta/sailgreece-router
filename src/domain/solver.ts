...
export function deriveAlternatives(
  ...
  for (const candidate of buildCandidates(snapshot, startIslandId)) {
    ...
    // Kein FEST etablierter roter Tag in einer angebotenen Alternative:
    // 'budget' ist bewusst keine Safety-Verletzung (liegen bleiben ist
    // sicher), aber ein Plan mit einem 9-h-Rot-Tag ist kein Angebot. Der
    // Zeuge oben bleibt unbedingt — trüge selbst er einen roten Tag, gäbe es
    // nichts Besseres, und Verschweigen bräche die Einlösbarkeit des Gelbs.
    if (validity.violations.some((v) => v.kind === 'budget' && v.assumed !== true))
      continue;
    seen.add(key);
    safe.push({
      plan,
