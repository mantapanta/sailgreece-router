...
import { legWindow, nightWindow } from './time.ts';
...
export function applyPersistenceAssumption(snapshot: PlanningSnapshot): {
  ...
  for (const [key, fc] of Object.entries(snapshot.forecast)) {
    ...
    const gustProfile = scalarProfile(fc.gustKn, hodOriginal);
    ...
    // Gusts and wave period are carried but do not drive verdicts — they are
    // filled for consistency, without owning the assumption flag.
    const gustKn = fillFromProfile(padded(fc.gustKn, times.length), gustProfile, hod, new Array(times.length).fill(false));
