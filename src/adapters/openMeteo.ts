...
/** Model run initialisation time — best effort, null when unavailable. */
async function fetchModelRunIso(model: string): Promise<string | null> {
  try {
    const resp = await fetch(`https://api.open-meteo.com/data/${model}/static/meta.json`);
...
export async function fetchForecastBundle(
  ...
  const marineTimes = (marineList?.[0]?.hourly?.['time'] as string[] | undefined) ?? [];
  marineTimes.forEach((t, i) => marineIndex.set(toIsoUtc(t), i));

  const forecast: Record<string, PointForecast> = {};
  locations.forEach((loc, li) => {
    const wind = forecastList[li];
    const marine = marineList?.[li];
    const windKn = wind ? seriesOf(wind, 'wind_speed_10m', times.length) : new Array(times.length).fill(null);
    const windDirDeg = wind ? seriesOf(wind, 'wind_direction_10m', times.length) : new Array(times.length).fill(null);
    const gustKn = wind ? seriesOf(wind, 'wind_gusts_10m', times.length) : new Array(times.length).fill(null);
