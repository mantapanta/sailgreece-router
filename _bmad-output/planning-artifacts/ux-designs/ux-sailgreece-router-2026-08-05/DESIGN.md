...
SailGreece Router is a private skipper's tool with the finish of a modern consumer travel product — the register is Airbnb-class polish, not marine-instrument or brochure. The reference composition is `mockups/direction-consumer-warm.html` (the chosen direction; this spine wins on conflict), and the current-state baseline it corrects is `imports/screenshot-mobile-tagesansicht-ist-zustand.png`.
...
- **Ampel badge** — `{components.ampel-badge}`: pill in the state's tint, 9px dot in the state hue (Gelb dots on light ground use `{colors.ampel-gelb-graphic}`), text label in the state's text color ("● Grün"). The only status badge in the system. **Never a bare dot without a label — no exemptions:** dense list rows carry the verdict word in `{typography.caption}` next to the dot ("Tag 4 · Serifos · Gelb ›"); color alone never carries the verdict, anywhere.
