# Review: Paros (`paros`)

Status: **FREIGEGEBEN** (`approved: true`)

Quelle der Datei: Brief-Addendum 2026-07-30 (Quelle 2: 'Parikia bei Meltemi gut geschützt', Naoussa, Piso Livadi; Parikia ~27 Fähranläufe/Tag im Juli) + Deep-Research Lauf 2 vom 2026-08-03 (Liegeplatz-Details, Ansteuerungswarnungen). Koordinaten und Sektorgeometrie aus dem Bestand beibehalten (Lauf 2 setzte Piso Livadi 1,6 sm nördlich, also bei Molos statt Piso Livadi), Stärkegrenzen auf den kleineren Wert beider Quellen gesetzt. OFFENER KONFLIKT bei Naoussa — siehe seeding/research/lauf-2-pruefprotokoll.md. | Gastro-Recherche 2026-08-06: Restaurant-Subebene für Naoussa ergänzt, Reservierungsnummer des Hafenvogts konkretisiert (weiterhin unverifiziert). NICHT übernommen: die Recherche weitet Naoussa auf 270–60 / 35 kn und confidence 'hoch' — der Bestandssektor 60–320 / 22 kn bewertet bewusst die riffreiche ANSTEUERUNG, nicht das geschützte Innenbecken.

> FR24: Sichere Liegeplätze zuerst prüfen — sie steuern die Nacht-Ampel. Sektorsemantik: „geschützt gegen Wind/Welle KOMMEND AUS fromDeg° im Uhrzeigersinn bis toDeg°", Grenzen inklusiv, Wrap über Nord erlaubt.

## Plätze (3)

### Parikia (`paros-parikia`, hafen)

**Sicherer Liegeplatz (sicherheitsrelevant — zuerst prüfen!):**

| Art | Sektor |
|---|---|
| Wind | geschützt aus 355°–250° (Wrap über Nord), bis 30 kn |
| Welle | geschützt aus 355°–250° (Wrap über Nord), bis 0.8 m |

Quelle: Quelle 2: 'expliziter Meltemi-Schutzhafen'; Bucht öffnet nach W; Fährschwell beachten. Lauf 2 gibt Öffnung nach WNW, Sektor 340–220, gleiche Windgrenze 30 kn, Welle 0.8 m — bestätigt den Bestand weitgehend. Wellengrenze auf 0.8 m gesenkt.

Koordinaten: 37.0853, 25.1519 · Qualitäten: Schönheit 3/5, Restaurant 4/5, Badestrand 3/5

Warnungen: ⚠ Starker Fährverkehr (~27 Anläufe/Tag im Juli) — Schwell und Rangieren am Anleger · ⚠ Klippen und Untiefen in der Bucht-Zufahrt (Lauf 2 nennt ein 'Super-Manois-Riff' — Bezeichnung unverifiziert, Ansteuerung nach Karte fahren) · ⚠ Bei starkem Meltemi läuft Nordwestschwell um das Kap in die Bucht · ⚠ Meltemi-sicher: bedingt

**Liegeplatz-Details** (Konfidenz: hoch):

| Angabe | Wert |
|---|---|
| Anlegeart | roemisch-katholisch |
| Wassertiefe | 2.5–4.5 m |
| Grössenlimit (LoA) | 30 m |
| Haltegrund | sand-schlamm |
| Haltequalität | gut — Gute Haltekraft auf Sand und Schlamm. |
| Seegras | Posidonia-Zonen außerhalb des Ankerfeldes. |
| Kapazität | 40 Yachten |
| Reservierbar | nein |
| Müllentsorgung | ja |
| Landstrom / Wasser | ja / ja |
| Diesel am Steg | ja |
| Duschen / WC | ja |
| Versorgung an Land | Vollständige Infrastruktur, Supermärkte, Bäckereien (1–3 Min) |
| Preis | ca. 15–22 EUR/Nacht (Gemeindehafen, 2024) |
| Hafengebühren | Hafenbehörde Paros, Einklarieren erforderlich |
| Schwell | Kräftiger Nordwestschwell läuft bei starkem Meltemi um das Kap in die Bucht. |
| Fährverkehr | Extrem hoher Fährverkehr Tag und Nacht. |
| Beiboot-Anlandung | Am Hafenkai oder Beibootsteg im Süden |
| Auflagen | Fährroute meiden, Fahrwasser beachten |
| UKW-Kanal | 12 |

Quellen: Deep-Research Lauf 2: Heikell, Greek Waters Pilot 14. Aufl., S. 250 (Seitenangabe unverifiziert)

### Naoussa (`paros-naoussa`, hafen)

**Sicherer Liegeplatz (sicherheitsrelevant — zuerst prüfen!):**

| Art | Sektor |
|---|---|
| Wind | geschützt aus 60°–320°, bis 22 kn |
| Welle | geschützt aus 60°–320°, bis 0.3 m |

Quelle: Quelle 2: Zielkatalog; große Bucht öffnet nach N — bei Meltemi exponiert. KONFLIKT: Lauf 2 gibt den nahezu invertierten Sektor 270–60 mit 35 kn an und begründet das mit dem geschützten INNENBECKEN hinter der Wellenbrechermole, urteilt für die Ansteuerung aber 'Meltemi-sicher: bedingt'. Bestandsgeometrie beibehalten (bewertet die exponierte Ansteuerung, nicht das Innenbecken), Wellengrenze auf 0.3 m gesenkt.

Koordinaten: 37.1236, 25.2375 · Qualitäten: Schönheit 5/5, Restaurant 5/5, Badestrand 4/5

Warnungen: ⚠ Ansteuerung erfordert genaue Navigation wegen vorgelagerter Riffe — bei Meltemi und Welle heikel · ⚠ Innenbecken laut Lauf 2 bei Meltemi ruhig; der Sektor bewertet bewusst die Ansteuerung, nicht den Liegeplatz · ⚠ Geringe Wassertiefe im Altstadt-Fischerbecken, Yachten liegen am Ostkai · ⚠ Meltemi-sicher: bedingt

**Liegeplatz-Details** (Konfidenz: mittel):

| Angabe | Wert |
|---|---|
| Anlegeart | murings |
| Wassertiefe | 2.2–3.5 m |
| Grössenlimit (LoA) | 20 m |
| Haltegrund | schlamm |
| Haltequalität | gut — Mooringleinen im neueren Yachthafenbecken. |
| Seegras | Im Innenbecken nicht relevant. |
| Kapazität | 30 Yachten |
| Reservierbar | ja — Telefon Hafenvogt Naoussa +30 22840 51225 (Nummer aus Lauf 2 und Gastro-Recherche 2026-08-06 gleichlautend, aber weiterhin unverifiziert — vor Verlass darauf gegenprüfen) |
| Müllentsorgung | ja |
| Landstrom / Wasser | ja / ja |
| Diesel am Steg | nein |
| Duschen / WC | ja |
| Versorgung an Land | Zahlreiche Geschäfte und Tavernen in Gehdistanz |
| Preis | ca. 20–30 EUR/Nacht (Gemeinde/Marina, 2024) |
| Hafengebühren | Liegegeld vor Ort von der Marina-Verwaltung |
| Schwell | Guter Schutz im Innenbecken hinter der Wellenbrechermole. |
| Fährverkehr | Keine Großfähren im Fischerhafen. |
| Beiboot-Anlandung | Direkt am Kai möglich |
| Auflagen | Sehr geringe Wassertiefe im Altstadt-Fischerbecken; Yachten nutzen den Ostkai |
| UKW-Kanal | 12 |

Quellen: Deep-Research Lauf 2: Heikell, Greek Waters Pilot 14. Aufl., S. 253 (Seitenangabe unverifiziert)

**Gastronomie (1)** — bewertet nichts, Anzeige nur:

| Lokal | Bewertung | Küche | Spezialitäten | Anlandung / Weg | Reservierung | Konfidenz |
|---|---|---|---|---|---|---|
| Sigi Ikthios (`paros-sigi-ikthios`) | 4.8/5 | Gehobenes Seafood und mediterrane Gourmetküche | Thunfisch-Carpaccio mit Trüffelöl, Hummer-Linguine | Direkt am alten Fischerhafen-Becken | Telefon +30 22840 52639, im August 2–3 Tage Vorlauf (Nummer unverifiziert) | mittel |

Quellen: Deep-Research Gastro 2026-08-06: Navily 2025; Deep-Research Gastro 2026-08-06: CruisersWiki Paros

### Piso Livadi (`paros-piso-livadi`, hafen)

**Sicherer Liegeplatz (sicherheitsrelevant — zuerst prüfen!):**

| Art | Sektor |
|---|---|
| Wind | geschützt aus 160°–70° (Wrap über Nord), bis 28 kn |
| Welle | geschützt aus 160°–70° (Wrap über Nord), bis 0.5 m |

Quelle: Quelle 2: Zielkatalog; kleiner Hafen an der Ostküste, öffnet nach SE. Lauf 2 bestätigt Öffnung nach S bis SE und Windgrenze 28 kn, gibt Sektor 270–120 und Welle 0.5 m. Bestandsgeometrie beibehalten, Wellengrenze auf 0.5 m gesenkt.

Koordinaten: 37.0106, 25.2617 · Qualitäten: Schönheit 3/5, Restaurant 3/5, Badestrand 4/5

Warnungen: ⚠ Platzangebot für Gastyachten stark limitiert (ca. 12 Boote) · ⚠ Grössenlimit laut Lauf 2 nur 16 m — für dieses Schiff knapp, vorab klären · ⚠ Geringe Wassertiefen im Innenbereich, Echolot beobachten · ⚠ Meltemi-sicher: ja

**Liegeplatz-Details** (Konfidenz: hoch):

| Angabe | Wert |
|---|---|
| Anlegeart | roemisch-katholisch |
| Wassertiefe | 2–3.2 m |
| Grössenlimit (LoA) | 16 m |
| Haltegrund | sand-schlamm |
| Haltequalität | gut — Zuverlässiges Halten im Hafenbecken. |
| Seegras | Vereinzelt Kraut außerhalb der Molen. |
| Kapazität | 12 Yachten |
| Reservierbar | nein |
| Müllentsorgung | ja |
| Landstrom / Wasser | ja / ja |
| Diesel am Steg | nein |
| Duschen / WC | nein |
| Versorgung an Land | Minimarkt und Tavernen am Hafen |
| Preis | ca. 10–14 EUR/Nacht (Gemeinde, 2024) |
| Hafengebühren | Gemeinde kassiert unregelmäßig |
| Schwell | Guter Schutz vor Meltemi, Schwell bei Süd- und Südostwind. |
| Fährverkehr | Nur Ausflugsboote nach Naxos/Koufonisia. |
| Beiboot-Anlandung | Am Molenkopf möglich |
| Auflagen | Geringe Wassertiefen im Innenbereich — Echolot beobachten |
| UKW-Kanal | — |

Quellen: Deep-Research Lauf 2: Heikell, Greek Waters Pilot 14. Aufl., S. 255 (Seitenangabe unverifiziert)

---
Freigabe: in `seeding/data/islands/paros.json` das Feld `approved` auf `true` setzen, dann `npm run seed:import`.
