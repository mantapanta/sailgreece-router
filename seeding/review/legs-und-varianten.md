...
| `folegandros--milos` | 34 sm | 9 | Langer offener Schlag (34 sm), keine Zwischenoption | — |
| `ios--santorin` | 21 sm | 3 | — | — |
| `iraklia--naxos` | 17.2 sm | 2 | Kurs rundet die Südwestecke von Naxos auf denselben Wegpunkten wie naxos--amorgos; Distanz 17.2 sm GEOMETRISCH als landfreier Kurs berechnet (keine recherchierte Quelle) — vor dem Törn verifizieren; Kalibrierung an naxos--amorgos: Geometrie 34.8 sm gegen 31 sm recherchiert, der Wert liegt also eher zu hoch | — |
| `kea--athen` | 36 sm | 4 | Düse Kea-Kanal (Lavrion–Makronisos): Modellwerte glätten die Beschleunigung | ursprünglich lavrion-basiert |
| `kea--kythnos` | 18 sm | 4 | — | — |
| `kea--syros` | 34 sm | 7 | Kafireas-/Andros-Sektor: Beschleunigung, Modellwerte zu niedrig | — |
| `koufonisia--amorgos` | 14 sm | 2 | Offener Schlag ohne Zwischenoption; macht Amorgos zum Durchgangsknoten statt zur Sackgasse; Distanz 14.0 sm GEOMETRISCH als landfreier Kurs berechnet (keine recherchierte Quelle) — vor dem Törn verifizieren; Kalibrierung an naxos--amorgos: Geometrie 34.8 sm gegen 31 sm recherchiert, der Wert liegt also eher zu hoch | — |
| `koufonisia--schinoussa` | 7.9 sm | 4 | Kurzschlag im Kleine-Kykladen-Archipel — auf dieser Distanz ist die 250-m-Landmaske grob, die Passage ist Pilotage nach Sicht und Seekarte; Distanz 7.9 sm GEOMETRISCH als landfreier Kurs berechnet (keine recherchierte Quelle) — vor dem Törn verifizieren; Kalibrierung an naxos--amorgos: Geometrie 34.8 sm gegen 31 sm recherchiert, der Wert liegt also eher zu hoch | — |
| `kythnos--athen` | 48 sm | 4 | Düse Kea-Kanal auf dem letzten Schlag; langer Törn (48 sm) | — |
| `kythnos--kea` | 18 sm | 4 | — | — |
| `kythnos--serifos` | 17 sm | 7 | — | — |
| `milos--polyaigos` | 8.1 sm | 6 | Distanz 8.1 sm GEOMETRISCH aus Koordinaten berechnet (keine Quellenangabe im Addendum) — Luftlinie unterschätzt den Seeweg, vor dem Törn verifizieren | — |
| `milos--sifnos` | 24 sm | 4 | — | — |
| `mykonos--paros` | 20 sm | 0 | Düse Mykonos–Paros: deutlich mehr Wind als der Modellwert | — |
| `naxos--amorgos` | 31 sm | 6 | Nur mit vorgedachtem Fähr-Exit über Naxos (Eskalationsleiter Quelle 1) | — |
| `naxos--koufonisia` | 21.9 sm | 3 | Kurs rundet die Südwestecke von Naxos auf denselben Wegpunkten wie naxos--amorgos; Distanz 21.9 sm GEOMETRISCH als landfreier Kurs berechnet (keine recherchierte Quelle) — vor dem Törn verifizieren; Kalibrierung an naxos--amorgos: Geometrie 34.8 sm gegen 31 sm recherchiert, der Wert liegt also eher zu hoch | — |
| `naxos--paros` | 12 sm | 6 | Düse Paros–Naxos: deutlich mehr Wind als der Modellwert | — |
| `naxos--santorin` | 51 sm | 7 | Langer Schlag, nur bei stabilem Fenster (Quelle 1: 8h30 / 6h50) | — |
| `paros--antiparos` | 10.8 sm | 5 | Düse Paros–Antiparos: der Kanal beschleunigt den Meltemi, Modellwerte glätten das; Distanz 10.8 sm GEOMETRISCH als landfreier Kurs berechnet (keine recherchierte Quelle) — vor dem Törn verifizieren; Kurs führt durch den flachen Antiparos-Kanal (Fahrrinne, Strömung, Pendelfähre Pounda–Antiparos-Stadt) — Pilotage, nicht Routing | — |
| `paros--ios` | 32 sm | 8 | — | — |
| `paros--naxos` | 12 sm | 6 | Düse Paros–Naxos: deutlich mehr Wind als der Modellwert | — |
| `paros--sifnos` | 26 sm | 4 | Insel-Beschleunigungszonen zwischen Sifnos und Paros | — |
| `paros--syros` | 24 sm | 2 | — | — |
| `polyaigos--paros` | 33 sm | 5 | — | — |
| `santorin--folegandros` | 25 sm | 2 | Offener Schlag westwärts, kein Schutz unterwegs | — |
| `schinoussa--iraklia` | 3.5 sm | 2 | Kurzschlag im Kleine-Kykladen-Archipel — auf dieser Distanz ist die 250-m-Landmaske grob, die Passage ist Pilotage nach Sicht und Seekarte; Distanz 3.5 sm GEOMETRISCH als landfreier Kurs berechnet (keine recherchierte Quelle) — vor dem Törn verifizieren; Kalibrierung an naxos--amorgos: Geometrie 34.8 sm gegen 31 sm recherchiert, der Wert liegt also eher zu hoch | — |
| `serifos--kythnos` | 17 sm | 7 | — | — |
| `serifos--sifnos` | 13 sm | 3 | — | — |
| `sifnos--antiparos` | 23.6 sm | 3 | Insel-Beschleunigungszonen zwischen Sifnos und Paros/Antiparos; Distanz 23.6 sm GEOMETRISCH als landfreier Kurs berechnet (keine recherchierte Quelle) — vor dem Törn verifizieren; Kurs rundet Sifnos im NORDEN wie paros--sifnos. Die Südumfahrung wäre 1 sm kürzer, führt aber dicht an Kitriani vorbei — das Inselchen fehlt in der 250-m-Landmaske, der Kurs sähe dort frei aus, wo Land liegt | — |
| `sifnos--milos` | 24 sm | 4 | — | — |
| `sifnos--paros` | 26 sm | 4 | Insel-Beschleunigungszonen zwischen Sifnos und Paros | — |
| `sifnos--serifos` | 13 sm | 3 | — | — |
| `syros--kythnos` | 27 sm | 4 | — | — |
| `syros--mykonos` | 21 sm | 5 | Andros/Tinos-Sektor: Düsenwirkung zwischen den Inseln | — |
