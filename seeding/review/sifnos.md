# Review: Sifnos (`sifnos`)

Status: **FREIGEGEBEN** (`approved: true`)

Quelle der Datei: Brief-Addendum 2026-07-30 (Quelle 2: Kamares; Buchten Fikiada, Vathy geschützt, Faros) + Deep-Research Lauf 2 vom 2026-08-03 (Liegeplatz-Details). Koordinaten und Sektorgeometrie aus dem Bestand beibehalten, Stärkegrenzen auf den kleineren Wert beider Quellen gesetzt. Kamares deckt sich in beiden Quellen vollständig. | Gastro-Recherche 2026-08-06: Restaurant-Subebene für Vathy und Kamares ergänzt, Gastro-Score Vathy 4 -> 5. NICHT übernommen: die Recherche weitet Vathy auf 300–200 (50° mehr Schutz nach Süd) und verschiebt beide Koordinaten um rund 0,7 sm — Geometrie und Sektoren des Bestands bleiben.

> FR24: Sichere Liegeplätze zuerst prüfen — sie steuern die Nacht-Ampel. Sektorsemantik: „geschützt gegen Wind/Welle KOMMEND AUS fromDeg° im Uhrzeigersinn bis toDeg°", Grenzen inklusiv, Wrap über Nord erlaubt.

## Plätze (3)

### Kamares (`sifnos-kamares`, hafen)

**Sicherer Liegeplatz (sicherheitsrelevant — zuerst prüfen!):**

| Art | Sektor |
|---|---|
| Wind | geschützt aus 330°–230° (Wrap über Nord), bis 30 kn |
| Welle | geschützt aus 330°–230° (Wrap über Nord), bis 1 m |

Quelle: Quelle 1: Rückfallhafen-Kette; Fjordbucht öffnet nach W. Lauf 2 (Heikell 14. Aufl. S. 241, CruisersWiki Sifnos) bestätigt Sektor, Grenzen und Öffnung nach W bis WSW unabhängig — beide Quellen deckungsgleich.

Koordinaten: 36.9903, 24.6603 · Qualitäten: Schönheit 4/5, Restaurant 4/5, Badestrand 4/5

Warnungen: ⚠ Unangenehmer Grundschwell im Hafenbecken bei starkem Meltemi, reflektiert an der Kaimauer · ⚠ Fallböen aus den Bergen · ⚠ Meltemi-sicher: bedingt · ⚠ Ruckdämpfer und lange Festmacher einplanen

**Liegeplatz-Details** (Konfidenz: hoch):

| Angabe | Wert |
|---|---|
| Anlegeart | roemisch-katholisch |
| Wassertiefe | 2.5–4.5 m |
| Grössenlimit (LoA) | 22 m |
| Haltegrund | sand |
| Haltequalität | gut — Fester Sandgrund. |
| Seegras | Vereinzelt Seegras im tieferen Wasser. |
| Kapazität | 20 Yachten |
| Reservierbar | nein |
| Müllentsorgung | ja |
| Landstrom / Wasser | ja / ja |
| Diesel am Steg | nein |
| Duschen / WC | ja |
| Versorgung an Land | Supermärkte und Bäckereien direkt an der Waterfront (2 Min) |
| Preis | ca. 10–15 EUR/Nacht (Gemeinde, 2024) |
| Hafengebühren | Hafenbehörde prüft Papiere |
| Schwell | Grundschwell reflektiert an der Kaimauer; Ruckdämpfer zwingend. |
| Fährverkehr | Fähren legen gegenüber an; erhebliche Schwellbildung bei Ankunft. |
| Beiboot-Anlandung | Am kleinen Holzsteg nahe dem Sandstrand |
| Auflagen | Fährplatz freihalten |
| UKW-Kanal | 12 |

Quellen: Deep-Research Lauf 2: CruisersWiki Sifnos; Deep-Research Lauf 2: Heikell, Greek Waters Pilot 14. Aufl.

**Gastronomie (1)** — bewertet nichts, Anzeige nur:

| Lokal | Bewertung | Küche | Spezialitäten | Anlandung / Weg | Reservierung | Konfidenz |
|---|---|---|---|---|---|---|
| Simos Restaurant (`sifnos-kamares-simos`) | 4.6/5 | Meeresfrüchte und ägäische Klassiker | Garnelen-Saganaki, Gegrillter Drachenkopf | Fußläufig vom Kai an der Hafenpromenade | Telefon +30 22840 31630, im August 1 Tag Vorlauf (Nummer unverifiziert) | niedrig |

Quellen: Deep-Research Gastro 2026-08-06: CruisersWiki Sifnos

### Vathy (`sifnos-vathy`, bucht)

**Sicherer Liegeplatz (sicherheitsrelevant — zuerst prüfen!):**

| Art | Sektor |
|---|---|
| Wind | geschützt aus 300°–250° (Wrap über Nord), bis 35 kn |
| Welle | geschützt aus 300°–250° (Wrap über Nord), bis 0.3 m |

Quelle: Quelle 2: 'Vathy geschützt' — fast geschlossene Bucht, schmale Öffnung nach W. Lauf 2 gibt die Öffnung nach SW und Sektor 300–200 an, gleiche Windgrenze 35 kn, aber nur 0.3 m Welle. Bestandsgeometrie beibehalten, Wellengrenze auf 0.3 m gesenkt.

Koordinaten: 36.9264, 24.6903 · Qualitäten: Schönheit 5/5, Restaurant 5/5, Badestrand 4/5

Warnungen: ⚠ Sehr starke Fallböen über die Kämme bei N/NE-Wind — das Schiff schwojt kurz heftig · ⚠ Meltemi-sicher: ja

**Liegeplatz-Details** (Konfidenz: hoch):

| Angabe | Wert |
|---|---|
| Anlegeart | anker-frei |
| Wassertiefe | 2.5–6 m |
| Grössenlimit (LoA) | 40 m |
| Haltegrund | sand |
| Haltequalität | gut — Sandboden bietet vorzügliche Haltekraft. |
| Seegras | Gut sichtbare Sandflecken zwischen kleineren Krautzonen. |
| Kapazität | 30 Yachten |
| Reservierbar | nein |
| Müllentsorgung | ja |
| Landstrom / Wasser | nein / nein |
| Diesel am Steg | nein |
| Duschen / WC | nein |
| Versorgung an Land | Kleine Minimärkte und Tavernen direkt am Strand |
| Preis | kostenfrei |
| Hafengebühren | keine |
| Schwell | Spiegelglattes Wasser bei Meltemi; nur die Böen bewegen das Schiff. |
| Fährverkehr | Kein Fährverkehr in der Bucht |
| Beiboot-Anlandung | Direkt am Strand oder am kleinen Kai bei der Kirche |
| Auflagen | — |
| UKW-Kanal | — |

Quellen: Deep-Research Lauf 2: Jim Baerselman, Cruising the South Aegean; Deep-Research Lauf 2: CruisersWiki Sifnos

**Gastronomie (2)** — bewertet nichts, Anzeige nur:

| Lokal | Bewertung | Küche | Spezialitäten | Anlandung / Weg | Reservierung | Konfidenz |
|---|---|---|---|---|---|---|
| Taverna Manolis (`sifnos-vathy-manolis`) | 4.8/5 | Traditionelle sifnische Küche | Revithada (im Tonofen geschmorte Kichererbsen), Mastelo (Lamm im Keramiktopf mit Rotwein und Dill) | Anlandung per Beiboot direkt am Sandstrand vor der Taverne | Telefon +30 22840 71115, im Hochsommer 2 Tage Vorlauf (Nummer unverifiziert) | mittel |
| Tsikali Taverna (`sifnos-vathy-tsikali`) | 4.7/5 | Familientaverne mit eigenen Hofprodukten | Sifnischer Manouri-Käse, Geschmortes Ziegenfleisch, Hausgemachter Ziegenkäse-Salat | Anlandung am Strand im Nordteil der Bucht | Telefon +30 22840 71123, 1 Tag Vorlauf (Nummer unverifiziert) | mittel |

Quellen: Deep-Research Gastro 2026-08-06: CruisersWiki Sifnos; Deep-Research Gastro 2026-08-06: Navily Community Guide 2025; Deep-Research Gastro 2026-08-06: Navily 2025

### Faros (`sifnos-faros`, bucht)

**Sicherer Liegeplatz (sicherheitsrelevant — zuerst prüfen!):**

| Art | Sektor |
|---|---|
| Wind | geschützt aus 210°–110° (Wrap über Nord), bis 25 kn |
| Welle | geschützt aus 210°–110° (Wrap über Nord), bis 0.4 m |

Quelle: Quelle 2: Zielkatalog; Bucht öffnet nach S/SE. Lauf 2 gibt die Öffnung nach S und den weiteren Sektor 270–90 mit 30 kn an. Bestandsgeometrie beibehalten (sie schließt SE-Lagen aus, was zu Quelle 2 passt), Wellengrenze auf 0.4 m gesenkt.

Koordinaten: 36.9394, 24.7458 · Qualitäten: Schönheit 4/5, Restaurant 3/5, Badestrand 4/5

Warnungen: ⚠ Bei Südwind völlig offen · ⚠ Meltemi-sicher: ja · ⚠ Badezonen im Sommer mit Bojen markiert

**Liegeplatz-Details** (Konfidenz: hoch):

| Angabe | Wert |
|---|---|
| Anlegeart | anker-frei |
| Wassertiefe | 3–7 m |
| Grössenlimit (LoA) | 30 m |
| Haltegrund | sand-seegras |
| Haltequalität | gut — In den sandigen Rinnen hervorragende Haltekraft. |
| Seegras | Seegrasfelder vorhanden; freie Sandstellen gezielt ansteuern. |
| Kapazität | 15 Yachten |
| Reservierbar | nein |
| Müllentsorgung | nein |
| Landstrom / Wasser | nein / nein |
| Diesel am Steg | nein |
| Duschen / WC | nein |
| Versorgung an Land | Kleine Tavernen und Minimarkt am Fischerhafen |
| Preis | kostenfrei |
| Hafengebühren | keine |
| Schwell | Sehr ruhiges Wasser bei Nordwinden. |
| Fährverkehr | Kein Fährverkehr |
| Beiboot-Anlandung | Am kleinen Fischersteg im Ort |
| Auflagen | Badezonen im Sommer mit Bojen markiert |
| UKW-Kanal | — |

Quellen: Deep-Research Lauf 2: Heikell, Greek Waters Pilot 14. Aufl., S. 244 (Seitenangabe unverifiziert)

---
Freigabe: in `seeding/data/islands/sifnos.json` das Feld `approved` auf `true` setzen, dann `npm run seed:import`.
